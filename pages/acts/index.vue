<template>
	<div>
		...
		<template v-else>
			...
			<ArticleCard v-else v-for="item in data" :item="item" :openLink="`/acts/${item.id}`"
...
const { data, pending: loading, refresh } = await useLazyFetch(`/api/acts`, {
	query: {
		search: searchText
	},
});
...
const deleteArticle = async (id) => {
	if (confirm(`Are you sure you want to delete this article`)) {
		const deleteResponse = await $fetch(`/api/acts/delete/${id}`, {
			method: 'DELETE'
		});
		refresh();
	}
